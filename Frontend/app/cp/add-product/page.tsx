...
export default function ProductForm() {
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0 && formData.colors.length > 0) {
      ...
      // Fallback: add to general images if no colors
  ...
  const addColor = () => {
    if (colorInput && !formData.colors.some(c => c.color === colorInput)) {
      setFormData(prev => ({ 
        ...prev, 
        colors: [...prev.colors, { color: colorInput, images: [], imagePreviews: [] }] 
      }));
      setColorInput('#000000');
      setSelectedColorIndex(formData.colors.length); // Auto-select new color
    }
  };
  ...
  try {
    ...
    // -------- Color Images (ALL under SAME KEY) --------
